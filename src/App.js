      ...
      <div className="bg-white shadow-md">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">ระบบบันทึกรายรับ-รายจ่าย</h1>
              <p className="text-gray-500 mt-1">G4S Payroll System</p>
            </div>
            <div className="flex items-center space-x-4">
              <button onClick={resetForm} className="bg-gray-300 hover:bg-gray-400 text-gray-800 px-5 py-2 rounded-lg font-semibold transition-colors">ล้างข้อมูล</button>
              <button onClick={() => setShowPrintPreview(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg font-semibold transition-colors">พิมพ์สลิป</button>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-lg p-6 shadow">
              ...
              <div className="space-y-4">
                {Object.entries({ 'ชื่อพนักงาน': 'employeeName', 'รหัสพนักงาน': 'employeeId', 'แผนก': 'department', 'เขต': 'zone', 'ประเภท': 'employeeType' }).map(([label, field]) => (
                  <div key={field}>
                    <label className="block text-sm font-medium text-gray-600 mb-1">{label}</label>
                    <input type="text" value={formData[field]} onChange={(e) => handleChange(field, e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-white rounded-lg p-6 shadow">
              ...
              <div className="space-y-4">
                {Object.entries({ 'งวดวันที่': 'payPeriod', 'งวดที่': 'periodNo', 'วันที่จ่าย': 'payDate' }).map(([label, field]) => (
                  <div key={field}>
                    <label className="block text-sm font-medium text-gray-600 mb-1">{label}</label>
                    <input type="text" value={formData[field]} onChange={(e) => handleChange(field, e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" />
                  </div>
            ...
            <div className="bg-white rounded-lg p-6 shadow">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {incomeFieldDetails.map(item => (
                    <div key={item.field} className="flex flex-col">
                      <label className="text-sm text-gray-600 mb-1">{item.label}</label>
                      <input type="text" value={formData[item.field]} onChange={(e) => handleChange(item.field, e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-right focus:ring-2 focus:ring-blue-500" placeholder="0.00" />
                    </div>
                  ))}
                </div>
                <div className="mt-4 pt-4 border-t">
                  <div className="flex justify-between items-center font-bold text-md">
                    ...
                    <span className="text-green-700">{formatNumber(totalIncome)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {deductionFieldDetails.map(item => (
                    <div key={item.field} className="flex flex-col">
                      <label className="text-sm text-gray-600 mb-1">{item.label}</label>
                      <input type="text" value={formData[item.field]} onChange={(e) => handleChange(item.field, e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-right focus:ring-2 focus:ring-blue-500" placeholder="0.00" />
                    </div>
                ...
                <div className="mt-4 pt-4 border-t">
                  <div className="flex justify-between items-center font-bold text-md">
                    ...
                    <span className="text-red-700">{formatNumber(totalDeduction)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center text-sm">
                  <div><div className="text-gray-500">รายได้สะสม</div><div className="font-bold text-gray-800">{formData.yearAccIncome}</div></div>
                  <div><div className="text-gray-500">ภาษีสะสม</div><div className="font-bold text-gray-800">{formData.yearAccTax}</div></div>
                  <div><div className="text-gray-500">ประกันสังคม</div><div className="font-bold text-gray-800">{formData.yearAccSoc}</div></div>
                  <div><div className="text-gray-500">เงินสด</div><div className="font-bold text-gray-800">{formData.cash}</div></div>
                </div>
                <div className="mt-4 pt-4 border-t">
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="text-sm text-gray-600">เงินได้สุทธิ</div>
                      <div className="text-xl font-bold text-green-600">{formatNumber(netIncome)}</div>
                    </div>
            ...
            <div className="p-6" ref={printRef}>
              <div className="border p-6">
                <div className="text-center mb-4">
                  ...
                  <p className="text-xs">588 ซอยศรีนครินทร์ 16 ถนนศรีนครินทร์ แขวงพัฒนาการ เขตสวนหลวง กรุงเทพมหานคร 10250</p>
                </div>
                <div className="text-center mb-6"><h2 className="text-2xl font-bold mb-1">ใบแจ้งรายได้ / PAY SLIP</h2></div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="pr-2 border-r">
                    <h3 className="text-md font-bold border-b pb-1 mb-2">รายการเงินได้</h3>
                    <div className="space-y-1 text-xs">
                      {incomeFieldDetails.filter(item => parseFloat(String(formData[item.field]).replace(/,/g, '')) > 0).map(item => (
                        <div key={item.field} className="flex justify-between"><span>{item.label}</span><span>{formatNumber(formData[item.field])}</span></div>
                      ))}
                    </div>
                  </div>
                  <div>
                    <h3 className="text-md font-bold border-b pb-1 mb-2">รายการเงินหัก</h3>
                    <div className="space-y-1 text-xs">
                      {deductionFieldDetails.filter(item => parseFloat(String(formData[item.field]).replace(/,/g, '')) > 0).map(item => (
                        <div key={item.field} className="flex justify-between"><span>{item.label}</span><span>{formatNumber(formData[item.field])}</span></div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 border-t pt-2">
                  <div className="font-bold flex justify-between pr-2 border-r"><span>ยอดรวมเงินได้</span><span>{formatNumber(totalIncome)}</span></div>
                  <div className="font-bold flex justify-between"><span>ยอดรวมเงินหัก</span><span>{formatNumber(totalDeduction)}</span></div>
                </div>
                <div className="text-right text-lg font-bold mt-4 border-t pt-2">เงินได้สุทธิ: {formatNumber(netIncome)}</div>
              </div>
            </div>
            <div className="flex justify-end p-4 space-x-4 bg-gray-100 rounded-b-lg">
              <button onClick={() => setShowPrintPreview(false)} className="px-5 py-2 bg-gray-400 text-white rounded-lg hover:bg-gray-500">ปิด</button>
              <button onClick={handleActualPrint} className="px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">พิมพ์</button>
            </div>
          </div>
        </div>
      )}
